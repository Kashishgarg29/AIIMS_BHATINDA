  ...
  return (
    <div className="fixed inset-0 z-[1000] bg-black/60 backdrop-blur-sm flex justify-center items-start overflow-y-auto w-full pt-10 pb-24 no-print sm:px-4">
      ...
      <div id="lab-print-container" className="w-full max-w-[210mm] min-h-[297mm] bg-white text-black p-8 sm:p-12 sm:border sm:border-slate-300 sm:shadow-2xl relative flex flex-col mx-auto shrink-0 print:border-none print:shadow-none print:m-0 print:p-0">
        ...
        <div className="text-center border-b-2 border-black pb-3 mb-5 pt-2 print:pt-0">
          <h1 className="text-2xl font-black uppercase tracking-widest text-black">
            AIIMS Bathinda School Health Camp
          </h1>
          {schoolName && (
            <h2 className="text-lg font-bold uppercase tracking-tight text-slate-700 mt-1">
              {schoolName}
            </h2>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8 bg-gray-50 p-4 border border-gray-200 rounded-lg print:bg-white print:border-black print:rounded-none">
          <div>
            ...
            <span className="font-bold text-sm uppercase whitespace-nowrap overflow-hidden text-ellipsis">{student.firstName} {student.lastName}</span>
          </div>
          <div>
            <span className="block text-[10px] uppercase font-black tracking-widest text-gray-500 print:text-black">Age & Gender</span>
            <span className="font-bold text-sm uppercase">{student.age} Y / {student.gender?.[0] || 'U'}</span>
          </div>
          <div>
            <span className="block text-[10px] uppercase font-black tracking-widest text-gray-500 print:text-black">Class / Section</span>
            <span className="font-bold text-sm uppercase">{student.classSec}</span>
          </div>
          <div>
          ...
          <div className="col-span-2 md:col-span-1 grid grid-cols-2 gap-2">
            <div>
              ...
              <span className="font-bold text-sm uppercase">{height} {height !== "NA" && "CM"}</span>
            </div>
            <div>
              <span className="block text-[10px] uppercase font-black tracking-widest text-gray-500 print:text-black">Weight</span>
              <span className="font-bold text-sm uppercase">{weight} {weight !== "NA" && "KG"}</span>
            </div>
          </div>
        </div>
        ...
        {/* Stamp Area */}
        <div className="mt-12 pt-8 border-t border-gray-300 flex justify-end break-inside-avoid">
